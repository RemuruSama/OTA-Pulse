...
class Request(
    ...
    companion object {
        fun validateResponse(responseCode: Int, responseBody: String) {
            if (responseCode != 200) throw RuntimeException("HTTP Error: $responseCode. Body: $responseBody")
            try {
                ...
                if (toCheck.optInt("responseCode", 200) != 200) {
                    throw RuntimeException("Server Error: ${toCheck.getInt("responseCode")} (${toCheck.optString("errMsg")})")
                }
            } catch (e: JSONException) {
                throw RuntimeException("Server returned non-JSON response: $responseBody", e)
            }
        }
        ...
        fun validateContent(content: JSONObject) {
            ...
            if (toCheck.has("checkFailReason") && toCheck.get("checkFailReason") != JSONObject.NULL) {
                throw RuntimeException("Update check failed: ${toCheck.getString("checkFailReason")}")
            }
        }
    }
}